...
void CommandAnalizer::setVC(id VC)
{
	viewController = VC;
}

void CommandAnalizer::increaseCommandsCount(int n)
{
	commandsCount+=n;
}

int CommandAnalizer::getCommandsCount()
{
	return commandsCount;
}


void CommandAnalizer::setCommandsCount(int n)
{
	commandsCount = n;
}
...
void CommandAnalizer::performMicroCommand(Register* microCommand)
{
	if (commandIndex < commandsCount) 
	{
		...
		F = F % 16;
		getResultReciever();
		[(mt1804ViewController*)viewController changeDataState:Y];
		getNextAddress();
	}
}
